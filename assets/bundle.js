var World888App = (() => {
  ...
  async function loadScript(src, onload, onerror) {
    ...
    const getApp = () => {
      if (typeof window.dc !== "undefined" && window.dc.app) return window.dc.app;
  ...
  // src/InputSystem.js
  var InputSystem = (() => {
    const ACTION_KEYS = /* @__PURE__ */ new Set([
      "w",
      "s",
      "a",
      "d",
    ...
    const IS_MAC = typeof navigator !== "undefined" ? navigator.platform.toUpperCase().includes("MAC") : false;
    let _scene = null;
    let _canvasRef = null;
    let _isLocked = false;
  ...
  // src/PhysicsSystem.js
  var PhysicsSystem = /* @__PURE__ */ (() => {
    ...
    function applyPhysicsToMesh2({ mesh, scene, shapeType = "BOX", options = { mass: 0, restitution: 0.1, friction: 0.5 } }) {
    ...
    function disposeCharacterController() {
      ...
      try {
        const ccScene = typeof _characterController.getScene === "function" ? _characterController.getScene() : null;
    ...
    return {
      initHavok,
      applyPhysicsToMesh: applyPhysicsToMesh2,
  ...
  function createPlayerState(id) {
    ...
    return {
      ...
      inputDirection: null,
      // Vector3(0,0,0) local space
      characterTargetOrientation: null,
      // Quaternion.Identity()
      ...
      slideVelocity: null,
      // Vector3
      slideDirectionIntentLocal: null,
      // Vector3
      ...
      bHopTimer: 0,
      // counts down after landing
      wantBHop: false,
      wallBounceTimer: 0,
      // window after wall collision
      wallBouncePreVel: null,
      // Vector3 snapshot before wall hit
      superGlideTimer: 0,
      // window at mantle peak
      superGlideActive: false,
      landingShockTimer: 0,
      // speed penalty duration after hard fall
      mantleTimer: 0,
      // rapid Y-rise detection
      lastPosY: null,
      lastInputDir: null,
      // for tap-strafe detection (prev frame)
      lastHorizVelWorld: null
      // for wall-bounce detection (Vector3)
    };
  }
  ...
  function createMovementConfig(normalHeight, crouchHeight, radius) {
    ...
    const cfg = {
      ...
      // ── Tap Strafe ───────────────────────────────────────────────────────────
      tapStrafeAirAccel: 30,
      tapStrafeMaxAdd: 3.8,
      tapStrafeMinSpeed: 3.5,
      // ── Wall Bounce ──────────────────────────────────────────────────────────
      wallBounceMinSpeed: 4,
      ...
      // ── Slope Sliding ────────────────────────────────────────────────────────
      slopeSlideAccelerationScale: 22,
      maxSlopeSlideSpeed: SPRINT * 2.4,
      minSlopeAngleForAccel: 2,
      // ── Fall → Slide ─────────────────────────────────────────────────────────
  ...
  // src/MovementSystem.js
  var MovementSystem = /* @__PURE__ */ (() => {
    let _ps = null;
    let _cfg = null;
    let _cc = null;
    let _cam = null;
    ...
    function _updateVisuals() {
      ...
      const scaleY = baseH > 0.01 ? targetH / baseH : 1;
    ...
    function _handleKeyUp({ key }) {
      ...
      switch (key) {
        case "w":
        case "arrowup":
          if (ps.inputDirection.z === 1) ps.inputDirection.z = 0;
          break;
        case "s":
        case "arrowdown":
          if (ps.inputDirection.z === -1) ps.inputDirection.z = 0;
        ...
        case "a":
        case "arrowleft":
          if (ps.inputDirection.x === -1) ps.inputDirection.x = 0;
          break;
        case "d":
        case "arrowright":
          if (ps.inputDirection.x === 1) ps.inputDirection.x = 0;
        ...
        case "j":
          ...
          break;
        case "c":
          _handleCrouchKeyUp("c");
          break;
        case "control":
          _handleCrouchKeyUp("control");
          break;
      }
    ...
    function _handleCrouchKeyDown(key) {
      ...
      const canSprintSlide = ps.state === "ON_GROUND" && (ps.isSprinting || ps.isShiftPressed && ps.inputDirection.z > 0) && ps.inputDirection.lengthSquared() > 0.1;
      if (canSprintSlide) {
        ps.state = "SLIDING";
        ps.isCrouching = true;
        ...
        ps.slideVelocity.addInPlace(fwd.scale(cfg.runSpeed * (cfg.slideInitialBoost - 1)));
    ...
    function _calculateVelocity(dt, supportInfo, currentVelocity) {
      ...
      inputWorld.y = 0;
      if (inputWorld.lengthSquared() > 1e-3) {
        const MathJS = Math;
      ...
      const m = _orientMatrix();
      const up = _upWorld();
      const forward = window.BABYLON.Vector3.TransformCoordinates(cfg.forwardLocalSpace, m);
      const camForward = window.BABYLON.Vector3.TransformCoordinates(cfg.forwardLocalSpace, camMatrix);
      const shockMult = ps.landingShockTimer > 0 ? cfg.landingShockSpeedMult : 1;
      let output = V.Zero();
      switch (ps.state) {
        // ── ON_GROUND ─────────────────────────────────────────────────────
        case "ON_GROUND": {
          ...
          if (sn.lengthSquared() < 1e-4) sn = up;
          ...
          if (fsn.lengthSquared() < 1e-4) fsn = up;
        ...
        case "SLIDING": {
          ...
          if (ps.justLandedIntoSlide) {
            const landingDir = (() => {
              ...
              const cv = currentVelocity.clone();
              cv.y = 0;
          ...
          if (sv.length() < cfg.slideMinSpeed) {
            ...
            if (ps.lastHorizVelWorld) {
              const prev2D = ps.lastHorizVelWorld.clone();
              prev2D.y = 0;
              const cur2D = sv.clone();
              cur2D.y = 0;
        ...
        case "START_JUMP": {
          ...
          if (wasSlide && ps.slideVelocity.lengthSquared() > 0.01) {
            const sv2d = ps.slideVelocity.clone();
            sv2d.y = 0;
            const boost = forward.scale(sv2d.length() * (cfg.slideJumpForwardBoostFactor - 1));
            ...
            const cur2D = currentVelocity.clone();
            cur2D.y = 0;
            ...
            const camFwd2D = camForward.clone();
            camFwd2D.y = 0;
            if (camFwd2D.length() > 1e-3) camFwd2D.normalize();
          ...
          if (ps.lastInputDir) {
            const curDir2D = ps.inputDirection.clone();
            curDir2D.y = 0;
            const changed = !curDir2D.equalsWithEpsilon(ps.lastInputDir, 0.05);
            const hspd = output.clone();
            hspd.y = 0;
        ...
        case "IN_AIR": {
          ...
          airTarget.y = 0;
          const curH = currentVelocity.clone();
          curH.y = 0;
          if (ps.lastInputDir) {
            const curDir2D = ps.inputDirection.clone();
            curDir2D.y = 0;
    ...
    function _onBeforeRender() {
      ...
      if (!_isLocked) return;
      const dt = Math.min(_scene.deltaTime / 1e3, 0.033);
    ...
    function initialize(scene, camera, characterController, displayCapsule, cfg) {
      ...
      _unsubs.push(EventBus.on("input:scroll", ({ direction }) => {
        if (direction === "down") _handleScrollDown();
      }));
      _unsubs.push(EventBus.on("input:tab", _handleTab));
      _unsubs.push(EventBus.on("input:pointerlock", ({ locked }) => {
        _isLocked = locked;
        _handlePointerLock({ locked });
      }));
      _renderObserver = scene.onBeforeRenderObservable.add(_onBeforeRender, void 0, true);
      _updateVisuals();
      return api;
    }
  ...
  var CameraSystem = /* @__PURE__ */ (() => {
    ...
    let _currentMode = "third";
    let _unsubs = [];
    const THIRD_DIST = 8;
    const THIRD_HEIGHT_OFFSET = 2.5;
    const THIRD_TARGET_OFFSET = 1;
    let FIRST_HEAD_OFFSET = null;
    const MIN_RAYCAST_DIST = 1;
    function _onBeforeRender() {
    ...
    function initialize(scene, canvasRef, displayCapsule, startPos) {
      ...
      _camera.minZ = 0.2;
      _camera.maxZ = 500;
      _camera.angularSensibility = 4e3;
      _camera.keysUp = [];
      _camera.keysDown = [];
      _camera.keysLeft = [];
      _camera.keysRight = [];
    ...
    function setMode(m) {
      _currentMode = m;
      EventBus.emit("camera:modeChange", { mode: m });
    }
    function dispose() {
  ...
  // src/MultiplayerSystem.js
  var MultiplayerSystem = /* @__PURE__ */ (() => {
    const CHANNEL_NAME = "obsidian-world-builder-sync";
    const SERVER_PORT = 8885;
    const SEND_INTERVAL = 50;
    const PRUNE_INTERVAL = 8e3;
    const STALE_THRESHOLD = 5e3;
    let _scene = null;
    let _movementSystem = null;
    let _passcode = null;
    ...
    const _remotePlayers = /* @__PURE__ */ new Map();
    let _unsubs = [];
    ...
    function _idToColor(id) {
      ...
      for (let i = 0; i < id.length; i++) {
        hash = hash * 31 + id.charCodeAt(i) & 16777215;
      }
    ...
    function _getOrCreateGhost(id) {
      ...
      const root2 = new window.BABYLON.TransformNode(`ghostRoot_${id}`, _scene);
      root2.setEnabled(false);
      const entry = {
        root: root2,
        mesh: null,
        // loaded GLB root mesh
        anims: {},
        // animation groups keyed by name
        currentAnim: null,
        lastSeen: Date.now(),
        lastState: null,
        color
      ...
      if (_catGlbUrl && window.BABYLON?.SceneLoader) {
        ...
        window.BABYLON.SceneLoader.ImportMeshAsync("", rootUrl, filename, _scene, null, ".glb").then((result) => {
          ...
          if (glbRoot) {
            glbRoot.parent = root2;
            glbRoot.position.y = -0.9;
          ...
          if (!_cleanedUp && _remotePlayers.has(id)) {
            const cap = window.BABYLON.MeshBuilder.CreateCapsule(
              `ghostCap_${id}`,
              { height: 1.8, radius: 0.5, subdivisions: 4 },
              _scene
            );
            cap.parent = root2;
    ...
    function _updateGhost(data) {
      ...
      const scaleY = data.isCrouching ? 0.5 : 1;
    ...
    function _removeGhost(id) {
      ...
      if (!entry) return;
      Object.values(entry.anims).forEach((ag) => {
        try {
          ag.stop();
          ag.dispose();
        } catch (_) {
    ...
    function _getLocalSnapshot() {
      ...
      return {
        ...
        orientation: ps.characterTargetOrientation ? {
          x: ps.characterTargetOrientation.x,
          y: ps.characterTargetOrientation.y,
          z: ps.characterTargetOrientation.z,
          w: ps.characterTargetOrientation.w
        } : { x: 0, y: 0, z: 0, w: 1 },
        ts: Date.now()
      };
    }
  ...
  var Engine = /* @__PURE__ */ (() => {
    function create() {
      ...
      async function _createCharacter(scene, catGlbUrl) {
        ...
        const START = new window.BABYLON.Vector3(-4, 33, -16);
        ...
        if (catGlbUrl) {
          try {
            ...
            if (rootNode) {
              ...
              rootNode.position.y = -NORMAL_H / 2;
              rootNode.rotation = new window.BABYLON.Vector3(0, Math.PI, 0);
            }
      ...
      function dispose() {
        ...
        try {
          ...
          console.warn("[Engine] Multiplayer dispose error:", e);
        }
        try {
          RenderSystem.dispose();
        } catch (e) {
          console.warn("[Engine] Render dispose error:", e);
        }
        try {
          MovementSystem.dispose();
        } catch (e) {
          console.warn("[Engine] Movement dispose error:", e);
        }
        try {
          CameraSystem.dispose();
        } catch (e) {
          console.warn("[Engine] Camera dispose error:", e);
        }
        try {
          InputSystem.dispose();
        } catch (e) {
          console.warn("[Engine] Input dispose error:", e);
        }
        try {
          PhysicsSystem.dispose();
        } catch (e) {
          console.warn("[Engine] Physics dispose error:", e);
        }
        EventBus.clear();
        if (_scene && !_scene.isDisposed) {
          try {
            ...
            console.warn("[Engine] Scene dispose error:", e);
          }
        }
        if (_babylonEngine && !_babylonEngine.isDisposed) {
          try {
            _babylonEngine.dispose();
          } catch (e) {
            console.warn("[Engine] Babylon engine dispose error:", e);
          }
        }
        _scene = null;
        _babylonEngine = null;
  ...
  function SpherePipSpawner({ scene, helperRef }) {
    (0, import_react.useEffect)(() => {
      ...
      const sphereConfigs = [
        {
          ...
          position: new window.BABYLON.Vector3(10, 8, -10),
          // Raised above ground
          pip: {
            filePath: fileName,
        ...
        },
        {
          ...
          position: new window.BABYLON.Vector3(13, 10, -12),
          // Raised above ground
          pip: {
            filePath: fileName,
            header: "ViewComponent",
        ...
        },
        {
          ...
          position: new window.BABYLON.Vector3(15, 9, -9),
          // Raised above ground
          pip: {
            filePath: fileName,
            header: "ViewComponent",
      ...
      sphereConfigs.forEach((config, index) => {
        ...
        if (mat.microSurface !== void 0) mat.microSurface = 0.9;
        mat.alpha = 0.96;
        sphere.material = mat;
        sphere._baseY = sphere.position.y;
        const obs = scene.onBeforeRenderObservable.add(() => {
          const t = performance.now() * 1e-3 + index * 0.3;
  ...
  function applyHavokPhysicsInternal(mesh, scene, shapeType = window.BABYLON.PhysicsShapeType.BOX, options = { mass: 0, restitution: 0.1, friction: 0.5 }) {
    if (!mesh || mesh.isDisposed()) {
      console.warn("ApplyPhysicsInternal: Null or disposed mesh.");
      return null;
    }
    if (!scene || scene.isDisposed || !scene.isPhysicsEnabled()) {
      console.warn(`ApplyPhysicsInternal: Scene invalid or physics not enabled for mesh ${mesh.name}.`);
      return null;
    }
    if (!window.BABYLON?.PhysicsAggregate) {
      console.error("ApplyPhysicsInternal: BABYLON.PhysicsAggregate not found.");
      return null;
    }
    ...
    try {
      ...
      if (!aggregate || !aggregate.body) {
        ...
        if (aggregate?.dispose) aggregate.dispose();
        mesh.physicsAggregate = null;
        return null;
      }
  ...
  function applyPhysicsToMesh({ mesh, scene, shapeType = "BOX", options = { mass: 0, restitution: 0.1, friction: 0.5 } }) {
    if (!window.BABYLON?.PhysicsShapeType) {
      console.error("ApplyPhysics: BABYLON.PhysicsShapeType not found.");
      return null;
    }
    ...
    if (selectedShapeType === void 0) {
      console.warn(`ApplyPhysics: Unknown shapeType "${shapeType}". Defaulting to BOX.`);
      return applyHavokPhysicsInternal(mesh, scene, window.BABYLON.PhysicsShapeType.BOX, options);
    }
  ...
  PaneLogic.loadLottie = function(loadScript2) {
  ...
  PaneLogic.createPane = async function({ scene, filePath, position = new window.BABYLON.Vector3(0, 2, 5), loadScript: loadScript2 }) {
    try {
      const mediaURL = await PaneLogic.requireMediaFile(filePath);
      const isLottie = filePath.toLowerCase().endsWith(".json");
      if (isLottie) {
        const lottie = await PaneLogic.loadLottie(loadScript2);
  ...
  PaneLogic.handleMediaInteractionForPane = function({ scene, pane, defaultFilePath, loadScript: loadScript2 }) {
  ...
  // src/SceneLoader.js
  async function _loadIntoScene(scene, glbConfig, folderPath3) {
    ...
    if (isBrowser) {
      ...
      const localCachePath = `${folderPath3}/assets/glb/${file}`;
    ...
    if (groundOptions?.enable) {
      groundMesh = window.BABYLON.MeshBuilder.CreateGround("sceneGround", {
        width: groundOptions.size || 1e3,
        height: groundOptions.size || 1e3,
    ...
    const animObserver = scene.onBeforeRenderObservable.add(() => {
      ...
      if (obeliskMesh) {
        ...
        hoverTime += eng.getDeltaTime() / 1e3 * HOVER_FREQ;
  ...
  var SceneLoader = { loadIntoScene: _loadIntoScene };

  // src/WorldLogic.js
  var folderPath = "";
  var BABYLON_URL = "https://cdn.babylonjs.com/babylon.js?v=7.5.0";
  var GLTF_URL = "https://cdn.babylonjs.com/loaders/babylon.glTFFileLoader.min.js?v=7.5.0";
  var HAVOK_UMD_URL = "https://cdn.babylonjs.com/havok/HavokPhysics_umd.js?v=7.5.0";
  var HAVOK_WASM_URL = "https://cdn.babylonjs.com/havok/HavokPhysics.wasm?v=7.5.0";
  var DEFAULT_GLB = {
  ...
  function WorldLogic({ canvasRef, glbBasePath = "assets/glb/", passcode = null }) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const { engine, scene, camera, displayCapsule, applyPhysicsToMesh: applyPhysicsToMesh2 } = engineResources;
        const { glbRootNode, environmentMeshes, animatedMeshes, sceneCleanup } = await _loadGLBIntoScene(scene, DEFAULT_GLB);
        if (glbRootNode) {
        ...
        for (const node of environmentMeshes) {
          ...
          const opts = isAnimated ? { mass: 0, restitution: 0.1, friction: 0.5, motionType: window.BABYLON.PhysicsMotionType.KINEMATIC } : { mass: 0, restitution: 0.1, friction: 0.5, motionType: window.BABYLON.PhysicsMotionType.STATIC };
          try {
            const agg = applyPhysicsToMesh2({ mesh: node, scene, shapeType: "MESH", options: opts });
            if (agg) {
              physicsCount++;
  ...
  // src/ScreenModeHelper.jsx
  var import_react2 = __toESM(require_react());
  var import_jsx_runtime = __toESM(require_jsx_runtime());
  var BrowserWindow = null;
  var os = null;
  try {
    const electron = __require("@electron/remote") || __require("electron").remote || {};
    BrowserWindow = electron.BrowserWindow;
    os = __require("os");
  } catch (e) {
  }
  var requestSecure = async (url, options = {}) => {
    try {
  ...
  var DEFAULT_FALLBACK_ZINDEX = 1e4;
  function updateHighestZIndex() {
    ...
    document.querySelectorAll(".fresh-pip").forEach((el) => {
      ...
      let z = computedZStr === "auto" || computedZStr === "" ? parseInt(el.style.zIndex, 10) || DEFAULT_FALLBACK_ZINDEX : parseInt(computedZStr, 10) || 0;
      if (z > max) {
        max = z;
      }
    });
  ...
  function bringToFront(container2, fallback = 0) {
    updateHighestZIndex();
    if (fallback && highestZIndex < fallback) {
      highestZIndex = fallback;
    }
    highestZIndex++;
    container2.style.setProperty("z-index", highestZIndex, "important");
    let computed = window.getComputedStyle(container2).zIndex;
    let forcedReflow = container2.offsetHeight;
  }
  function updateCanvasSize(container2) {
    const canvas = container2.querySelector("canvas");
    if (canvas) {
      canvas.style.width = "100%";
      canvas.style.height = "100%";
      const newWidth = container2.clientWidth;
      const newHeight = container2.clientHeight;
  ...
  function resetScreenMode(container2, defaultStyle, originalParentRefForWindow, originalParentRefForPiP) {
    if (originalParentRefForWindow.current) {
      originalParentRefForWindow.current.appendChild(container2);
      originalParentRefForWindow.current = null;
    }
    if (originalParentRefForPiP.current) {
      originalParentRefForPiP.current.appendChild(container2);
      originalParentRefForPiP.current = null;
      if (container2._pipDragAttached) {
        container2.removeEventListener("mousedown", container2._pipDragAttached.dragStart);
        window.removeEventListener("mousemove", container2._pipDragAttached.dragMove);
        window.removeEventListener("mouseup", container2._pipDragAttached.dragEnd);
        delete container2._pipDragAttached;
        delete container2._pipDragging;
      }
      if (container2._pipResizers) {
        container2._pipResizers.forEach((handle) => handle.remove());
        delete container2._pipResizers;
      }
      delete container2._pipReset;
    }
    container2.style.cssText = defaultStyle;
    let forcedReflow = container2.offsetHeight;
  }
  function applyBrowserMode(container2) {
    if (!document.fullscreenElement) {
      const fullscreenPromise = container2.requestFullscreen?.() || container2.webkitRequestFullscreen?.() || container2.mozRequestFullScreen?.() || container2.msRequestFullscreen?.();
      if (fullscreenPromise && fullscreenPromise.then) {
        fullscreenPromise.then(() => console.log("[applyBrowserMode] Fullscreen entered successfully")).catch((err) => console.error("[applyBrowserMode] Fullscreen request failed:", err));
      }
    } else {
      document.exitFullscreen?.();
    }
  }
  function applyWindowStyle(container2) {
    ...
    let reflowValue = container2.offsetHeight;
    updateCanvasSize(container2);
    bringToFront(container2, 9999);
    setTimeout(() => {
  ...
  function applyPipStyle(container2) {
    Object.assign(container2.style, {
      position: "fixed",
    ...
    let forced = container2.offsetHeight;
  }
  function applyScreenMode(mode, container2, originalParentRefForWindow, originalParentRefForPiP, defaultStyle) {
    if (!container2) return;
    const tokens = mode.trim().split(/\s+/);
    if (tokens.includes("reset")) {
      resetScreenMode(container2, defaultStyle, originalParentRefForWindow, originalParentRefForPiP);
      return;
    }
    if (tokens.includes("browser")) {
      applyBrowserMode(container2);
      return;
    }
    if (tokens.includes("window") || tokens.includes("pip")) {
      if (tokens.includes("window")) {
        if (!originalParentRefForWindow.current) {
          originalParentRefForWindow.current = container2.parentNode;
        }
        document.body.appendChild(container2);
        applyWindowStyle(container2);
      }
      if (tokens.includes("pip")) {
        if (!originalParentRefForPiP.current) {
          originalParentRefForPiP.current = container2.parentNode;
        }
        document.body.appendChild(container2);
        container2._pipReset = function() {
          resetScreenMode(container2, defaultStyle, originalParentRefForWindow, originalParentRefForPiP);
        };
        applyPipStyle(container2);
        setupPipDrag(container2);
        setupPipCornerResizers(container2);
      }
    }
  }
  function setupPipDrag(container2) {
    if (container2._pipDragAttached) return;
    const dragHandlers = {
      dragStart: (e) => {
        bringToFront(container2);
        container2._active = true;
        container2._pipDragging = true;
        container2._pipStartX = e.clientX;
        container2._pipStartY = e.clientY;
        container2._pipOrigTop = parseInt(getComputedStyle(container2).top, 10) || 0;
        container2._pipOrigLeft = parseInt(getComputedStyle(container2).left, 10) || 0;
      },
      dragMove: (e) => {
        if (!container2._pipDragging) return;
        const deltaX = e.clientX - container2._pipStartX;
        const deltaY = e.clientY - container2._pipStartY;
        container2.style.top = `${container2._pipOrigTop + deltaY}px`;
        container2.style.left = `${container2._pipOrigLeft + deltaX}px`;
      },
      dragEnd: (e) => {
        container2._pipDragging = false;
        container2._active = false;
        setTimeout(() => {
          bringToFront(container2);
        }, 0);
      }
    };
    container2.addEventListener("mousedown", dragHandlers.dragStart);
    window.addEventListener("mousemove", dragHandlers.dragMove);
    window.addEventListener("mouseup", dragHandlers.dragEnd);
    container2._pipDragAttached = dragHandlers;
  }
  function setupPipCornerResizers(container2) {
    if (container2._pipResizers) return;
    const corners = [
    ...
    corners.forEach(({ corner, style }) => {
      ...
      resizer.addEventListener("mousedown", (e) => {
        ...
        const computed = getComputedStyle(container2);
      ...
      container2.appendChild(resizer);
    });
    container2._pipResizers = resizers;
    const resizeMove = (e) => {
      if (!container2._pipResizers) return;
      container2._pipResizers.forEach((resizer) => {
        ...
        container2.style.height = `${newHeight}px`;
  ...
  function FreshPip({ onClose, filePath, functionName, customStyle = {} }) {
    ...
    const [LoadedComponent, setLoadedComponent] = (0, import_react2.useState)(null);
    (0, import_react2.useEffect)(() => {
      (async () => {
        try {
          const dynamicModule = await window.dc.require(filePath);
    ...
    (0, import_react2.useEffect)(() => {
      ...
      if (container2) {
        container2.addEventListener("pointerdown", () => {
          bringToFront(container2);
          container2._active = true;
        }, true);
        setupPipDrag(container2);
    ...
    return h(
      ...
      LoadedComponent ? h(LoadedComponent, {
  ...
  async function createExternalWindow() {
    if (!BrowserWindow) {
      ...
      if (typeof Notice !== "undefined") new Notice("External window mode requires Electron with remote module enabled.", 5e3);
      return null;
    }
    try {
      ...
      const externalWindow = new BrowserWindow({
        ...
        vibrancy: isMac ? "ultra-dark" : void 0,
        hasShadow: true,
        webPreferences: {
      ...
      const activeFile2 = "";
      const folderPath3 = "";
      const absFolderPath = window.dc.app.vault.adapter.getFullPath ? window.dc.app.vault.adapter.getFullPath(folderPath3) : window.dc.app.vault.adapter.basePath + "/" + folderPath3;
      ...
      externalWindow.once("ready-to-show", () => {
        externalWindow.show();
        if (typeof Notice !== "undefined") new Notice("\u2728 External window opened!", 3e3);
      });
      ...
      if (typeof Notice !== "undefined") new Notice("Failed to open external window: " + error.message, 5e3);
      return null;
    }
  }
  var WEB_SERVER_PORT = 8885;
  var W888_SERVER_VERSION = "v3-sse";
  ...
  function startWorldServer(assetsFolder) {
    ...
    try {
      let broadcast = function(excludeId, data) {
      ...
      const server = http.createServer((req, res) => {
        ...
        if (req.method === "POST" && pn === "/leave") {
          readBody(req).then((b) => {
            if (b.id) {
              players.delete(b.id);
              subscribers.delete(b.id);
              broadcast(b.id, { type: "PLAYER_LEFT", senderId: b.id });
            }
            res.writeHead(200);
            res.end("ok");
          });
        ...
        if (req.method === "GET" && pn === "/status") {
          let lanIP = "localhost";
          try {
            for (const ifaces of Object.values(os2.networkInterfaces()))
...
`);
        ...
        const filePath = pn === "/" || pn === "/index.html" ? path.join(assetsFolder, "player_viewer.html") : path.join(assetsFolder, pn);
      ...
      server.on("error", (err) => {
        if (err.code === "EADDRINUSE") {
          requestSecure(`http://localhost:${WEB_SERVER_PORT}/status`).then((r) => r.json()).then((data) => {
            if (data.ok) {
  ...
  var ScreenModeHelper = ({
    ...
    engine,
    onModeChange
    // optional callback to inform parent (WorldView) about mode changes
  }) => {
    const [activeMode, setActiveMode] = (0, import_react2.useState)(allowedScreenModes.includes(initialMode) ? initialMode : "default");
    const previousModeRef = (0, import_react2.useRef)("default");
    const activeModeRef = (0, import_react2.useRef)(activeMode);
    const isEnteringFullscreenRef = (0, import_react2.useRef)(false);
    (0, import_react2.useEffect)(() => {
      activeModeRef.current = activeMode;
    }, [activeMode]);
    const [serverOnline, setServerOnline] = (0, import_react2.useState)(false);
    const [serverPlayers, setServerPlayers] = (0, import_react2.useState)(0);
    const [serverPasscode, setServerPasscode] = (0, import_react2.useState)("");
    const [lanURL, setLanURL] = (0, import_react2.useState)("");
    (0, import_react2.useEffect)(() => {
      const adapter2 = window.dc?.app?.vault?.adapter;
      const isBrowserMode2 = !adapter2 || typeof adapter2.exists !== "function";
      if (isBrowserMode2) return;
      let mounted = true;
      requestSecure(`http://localhost:${WEB_SERVER_PORT}/status`).then((r) => r.json()).then((d) => {
        if (mounted) {
    ...
    const stopWorldServer = (0, import_react2.useCallback)(() => {
      ...
      try {
        ...
        console.error("[World888] Stop error:", e);
      }
    }, []);
    const toggleServer = (0, import_react2.useCallback)(() => {
      if (serverOnline) {
        stopWorldServer();
      } else {
        try {
          ...
          console.error("[World888] Start error:", e);
        }
      }
    }, [serverOnline, stopWorldServer]);
    const externalWindowRef = (0, import_react2.useRef)(null);
    const openExternalWindow = (0, import_react2.useCallback)(async () => {
    ...
    const closeExternalWindow = (0, import_react2.useCallback)(() => {
      if (externalWindowRef.current && !externalWindowRef.current.isDestroyed()) {
    ...
    const toggleMode = async (mode) => {
      ...
      if (mode === "pip") {
        ...
        try {
          ...
          try {
            const shell = __require("electron").shell;
            shell.openExternal(playerUrl);
          } catch (_) {
            const remote = __require("@electron/remote") || __require("electron").remote;
            if (remote?.shell) {
...
LAN Invite: ${inviteUrl}`, 8e3);
        ...
        try {
          ...
          if (typeof Notice !== "undefined") new Notice("\u{1F310} Opened player in web browser!", 3e3);
        } catch (e) {
          ...
          resizeTimeout = setTimeout(() => {
            entries.forEach((entry) => {
              ...
              if (activeMode === "pip") {
                ...
                scalingFactor = Math.max(1e-3, scalingFactor);
              }
      ...
      const checkExternalWindow = () => {
        ...
        if (extWindow && extWindow.isDestroyed()) {
          ...
          if (!requestedMode) {
            try {
              ...
              if (stored) {
                ...
                if (Date.now() - data.timestamp < 5e3) {
          ...
          if (requestedMode && requestedMode !== "window") {
            ...
            setActiveMode("default");
            try {
              if (typeof onModeChange === "function") onModeChange("default");
            } catch (e) {
            }
          }
        }
    ...
    const modeIcons = {
      ...
      // Show minimize when active, maximize when inactive
      window: "square",
    ...
    const buttonStyle = (isActive) => ({
      ...
      pointerEvents: "auto",
      // Ensure buttons can receive clicks
      userSelect: "none",
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("style", { children: `@keyframes pulse888{0%,100%{opacity:1;box-shadow:0 0 6px #4ade80}50%{opacity:.5;box-shadow:0 0 14px #4ade80}}` }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
        "div",
        {
          ...
          children: [
                  ...
                  children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    window.dc.Icon,
                    {
                      ...
                      style: {
                        ...
                        pointerEvents: "none"
                        // Let clicks pass through icon to button
                      }
                    }
                  )
                },
            ...
            activeMode === "pip" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
              "button",
              {
                type: "button",
                onMouseDown: (e) => {
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: {
              ...
              /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
                "div",
                {
                  title: serverOnline ? `World888 server online \xB7 ${serverPlayers} player${serverPlayers !== 1 ? "s" : ""} \xB7 http://localhost:${WEB_SERVER_PORT}` : "World888 server offline",
                  style: {
                    display: "flex",
                    alignItems: "center",
                    gap: "5px",
                    ...
                    cursor: "default",
                    userSelect: "none",
                    background: serverOnline ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.12)",
                    border: serverOnline ? "1px solid rgba(34,197,94,0.4)" : "1px solid rgba(239,68,68,0.3)",
                  ...
                  children: [
                    ...
                    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: serverOnline ? `:${WEB_SERVER_PORT}` : "Offline" }),
                    serverOnline && serverPlayers > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { style: {
              ...
              serverOnline && lanURL && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                "button",
                {
                  type: "button",
                  onMouseDown: (e) => {
                    ...
                    if (navigator.clipboard) {
                      navigator.clipboard.writeText(inviteLink).then(() => {
                        if (typeof Notice !== "undefined") new Notice("\u2728 LAN Invite Link copied to clipboard!", 3e3);
                      }).catch(() => {
              ...
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                "button",
                {
                  ...
                  style: {
                    ...
                    height: "36px",
                    marginRight: 0,
                    background: serverOnline ? "rgba(239,68,68,0.15)" : "rgba(34,197,94,0.12)",
                    border: serverOnline ? "1px solid rgba(239,68,68,0.35)" : "1px solid rgba(34,197,94,0.35)"
                  },
                  children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                    window.dc.Icon,
                    {
  ...
  // src/PreventDefaultInputs.js
  var import_react3 = __toESM(require_react());
  function preventDefaultInputs({ viewRef }) {
    const [isFocused, setIsFocused] = (0, import_react3.useState)(false);
    const isFocusedRef = (0, import_react3.useRef)(false);
    const originalCommandsRef = (0, import_react3.useRef)(null);
    const originalExecuteCommandRef = (0, import_react3.useRef)(null);
    const originalExecuteRef = (0, import_react3.useRef)(null);
    ...
    const handleFocus = () => {
      if (!window.dc.app || !window.dc.app.commands) {
      ...
      if (!originalCommandsRef.current) {
        originalCommandsRef.current = { ...window.dc.app.commands.commands };
      }
      if (!originalExecuteCommandRef.current) {
        originalExecuteCommandRef.current = window.dc.app.commands.executeCommandById;
      }
      if (!originalExecuteRef.current) {
        originalExecuteRef.current = window.dc.app.commands.execute;
      }
      window.dc.app.commands.commands = {};
      window.dc.app.commands.executeCommandById = (commandId) => {
        return false;
      };
      window.dc.app.commands.execute = (command) => {
        return false;
      };
    ...
    const restoreCommands = () => {
      ...
      if (window.dc.app && window.dc.app.commands) {
        if (originalCommandsRef.current) {
          window.dc.app.commands.commands = { ...originalCommandsRef.current };
          originalCommandsRef.current = null;
        }
        if (originalExecuteCommandRef.current) {
          window.dc.app.commands.executeCommandById = originalExecuteCommandRef.current;
          originalExecuteCommandRef.current = null;
        }
        if (originalExecuteRef.current) {
          window.dc.app.commands.execute = originalExecuteRef.current;
  ...
  // src/LoadingConfirmation.jsx
  var import_react4 = __toESM(require_react());
  var import_jsx_runtime2 = __toESM(require_jsx_runtime());
  function LoadingConfirmation({ onConfirm, onCancel }) {
    const [status, setStatus] = (0, import_react4.useState)("checking");
    const [assetsExist, setAssetsExist] = (0, import_react4.useState)(false);
    ...
    if (status === "checking") {
      return /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { style: {
        ...
        /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { style: {
          ...
          fontWeight: "700",
          marginBottom: "12px"
        }, children: "Checking Assets" }),
        /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { style: {
          color: "rgba(255, 255, 255, 0.4)",
          fontSize: "13px"
        }, children: "Verifying local cache..." })
      ] }) });
    }
    return /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { style: {
      position: "absolute",
      ...
      /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("h2", { style: {
        color: "#ffffff",
        fontSize: "32px",
        fontWeight: "700",
        marginBottom: "16px",
      ...
      status === "ready" && /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { style: {
        backgroundColor: "#000000",
        border: "1px solid rgba(139, 92, 246, 0.15)",
        borderRadius: "12px",
        padding: "16px 20px",
        ...
        /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { style: { flex: 1 }, children: [
          ...
          /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { style: {
            ...
            "Source: ",
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { style: { color: "rgba(255, 255, 255, 0.6)" }, children: "beto.assets/DATACORE/WORLD888" }),
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("br", {}),
            "Cache: ",
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { style: { color: "rgba(255, 255, 255, 0.6)" }, children: "assets/glb/" })
          ] })
        ] })
      ] }),
      status === "ready" && /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { style: {
        ...
        /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)(
          "button",
          {
            ...
            children: [
              /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(window.dc.Icon, { icon: "x", style: { width: "16px", height: "16px" } }),
              "Cancel"
            ]
          }
        ),
        /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)(
          "button",
          {
            ...
            children: [
              /* @__PURE__ */ (0, import_jsx_runtime2.jsx)(window.dc.Icon, { icon: "download", style: { width: "16px", height: "16px" } }),
              "Load World"
            ]
          }
        )
      ] }),
      (status === "checking" || status === "downloading") && /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { style: {
        marginTop: "8px",
        display: "flex",
        justifyContent: "center",
        alignItems: "center",
  ...
  function WorldView(props = {}) {
    ...
    const originalParentRefForWindow = (0, import_react5.useRef)(null);
    const originalParentRefForPiP = (0, import_react5.useRef)(null);
    const [worldResources, setWorldResources] = (0, import_react5.useState)(null);
    const worldResourcesRef = (0, import_react5.useRef)(null);
    const [isFullTab, setIsFullTab] = (0, import_react5.useState)(true);
    const [showLoadingConfirm, setShowLoadingConfirm] = (0, import_react5.useState)(true);
    const [isLoadingWorld, setIsLoadingWorld] = (0, import_react5.useState)(false);
    const [loadError, setLoadError] = (0, import_react5.useState)(null);
    const stateRefs = (0, import_react5.useRef)({}).current;
    const [passcode, setPasscode] = (0, import_react5.useState)(_initialPasscode || null);
    const [passcodeError, setPasscodeError] = (0, import_react5.useState)(null);
    const [showPasscodeModal, setShowPasscodeModal] = (0, import_react5.useState)(false);
    const [checkingStatus, setCheckingStatus] = (0, import_react5.useState)(_initialPasscode ? false : true);
    const [isHost, setIsHost] = (0, import_react5.useState)(false);
    const glbBasePath = (0, import_react5.useMemo)(() => {
      return `${folderPath2}/assets/glb/`;
    }, []);
    (0, import_react5.useEffect)(() => {
      ...
      const queryPasscode = urlParams2.get("passcode");
      if (queryPasscode) {
      ...
      const requestSecure2 = async (url, options = {}) => {
        try {
          if (typeof window !== "undefined" && typeof window.require === "function") {
            const obsidian = window.require("obsidian");
            if (obsidian && typeof obsidian.requestUrl === "function") {
      ...
      requestSecure2(`http://${host}:8885/status`).then((res) => res.json()).then((data) => {
        if (data.ok) {
          if (data.passcode) {
    ...
    (0, import_react5.useEffect)(() => {
      ...
      stateRefs.originalParent = container2.parentNode;
      stateRefs.placeholder = document.createElement("div");
      stateRefs.placeholder.style.display = "none";
      container2.parentNode.insertBefore(stateRefs.placeholder, container2);
      ...
      contentWrapper.appendChild(container2);
      Object.assign(container2.style, {
        position: "absolute",
        top: "0",
        left: "0",
      ...
      return () => {
        if (document.fullscreenElement === container2) {
          return;
        }
        if (container2.parentNode === document.body) {
          return;
        }
    ...
    (0, import_react5.useEffect)(() => {
      ...
      WorldLogic({ canvasRef, glbBasePath, passcode }).then((resources) => {
        if (isMounted) {
    ...
    }, [glbBasePath, isLoadingWorld, showLoadingConfirm, checkingStatus, showPasscodeModal, passcode]);
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)(
      "div",
      {
        ...
        style: {
          ...
          background: "#000",
          cursor: "crosshair"
          // Subtle crosshair cursor on the container
        },
        children: [
          showPasscodeModal && /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { style: {
            position: "absolute",
            top: 0,
            left: 0,
            width: "100%",
            ...
            color: "#ffffff"
          }, children: /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("form", { onSubmit: handlePasscodeSubmit, style: {
            ...
            /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { children: [
              ...
              /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("p", { style: {
                ...
                margin: 0,
                lineHeight: "1.5"
              }, children: "Enter Room Passcode to join multiplayer" })
            ] }),
            /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: "8px", textAlign: "left" }, children: [
              /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("label", { htmlFor: "passcodeInput", style: {
                fontSize: "11px",
                textTransform: "uppercase",
                letterSpacing: "1px",
                fontWeight: "600",
                color: "rgba(255, 255, 255, 0.4)"
              }, children: "Passcode (Format: 888-XXXX)" }),
              ...
              passcodeError && /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { style: {
                color: "#ef4444",
                fontSize: "13px",
                fontWeight: "500",
                marginTop: "4px",
          ...
          showLoadingConfirm && !showPasscodeModal && /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(
            LoadingConfirmation,
            {
          ...
          worldResources ? /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)(import_jsx_runtime3.Fragment, { children: [
            worldResources.scene && worldResources.SpherePipSpawner && /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(worldResources.SpherePipSpawner, { scene: worldResources.scene, helperRef }),
            worldResources.engine && /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(
              ScreenModeHelper,
              {
                helperRef,
                containerRef,
            ...
            /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { style: {
              ...
              /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("span", { style: { fontWeight: "500" }, children: worldResources.multiplayerResources ? `Multiplayer Active (${worldResources.multiplayerResources.instanceId.slice(-6)})${passcode ? ` \xB7 Room: ${passcode}` : ""}` : "Single Player" })
            ] })
          ] }) : isLoadingWorld && !showLoadingConfirm ? /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { style: {
            position: "absolute",
            top: "50%",
            left: "50%",
            ...
            /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { style: {
              ...
              color: "#ffffff",
              fontFamily: "system-ui, -apple-system, sans-serif"
            }, children: "Initializing World 888" }),
            loadError ? /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { style: {
              color: "#ef4444",
              ...
              /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("strong", { children: "Error Loading World:" }),
              /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("br", {}),
              loadError,
              /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { style: { marginTop: "12px", fontSize: "12px", color: "rgba(255,255,255,0.6)" }, children: "If you opened this file directly in a browser, please ensure you run the local server first (`node server/world888-server.js`) to serve the GLB files." })
              ...
              /* @__PURE__ */ (0, import_jsx_runtime3.jsxs)("div", { style: {
                ...
                /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { style: { width: "6px", height: "6px", borderRadius: "50%", background: "#8b5cf6", animation: "bounce 1.4s infinite ease-in-out both", animationDelay: "-0.32s" } }),
                /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { style: { width: "6px", height: "6px", borderRadius: "50%", background: "#8b5cf6", animation: "bounce 1.4s infinite ease-in-out both", animationDelay: "-0.16s" } }),
                /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("div", { style: { width: "6px", height: "6px", borderRadius: "50%", background: "#8b5cf6", animation: "bounce 1.4s infinite ease-in-out both" } })
              ] }),
              /* @__PURE__ */ (0, import_jsx_runtime3.jsx)("span", { style: {
                ...
                marginTop: "8px",
                fontFamily: "system-ui, -apple-system, sans-serif"
              }, children: "Building environment and loading assets..." })
            ] }),
  ...
  var LucideIcon = ({ icon, style }) => {
    const [svgContent, setSvgContent] = import_react6.default.useState("");
    const color = style?.color || "currentColor";
    const size = style?.fontSize || style?.width || "20px";
    import_react6.default.useEffect(() => {
      let active = true;
      fetch(`https://unpkg.com/lucide-static@latest/icons/${icon}.svg`).then((res) => {
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(
      "span",
      {
        style: {
          ...
          width: size,
          height: size,
          animation: isSpinning ? "spin 2s linear infinite" : void 0,
          ...style,
          color: void 0
        },
  ...
  var urlPasscode = urlParams.get("passcode");
  if (urlPasscode) {
    localStorage.setItem("w888_passcode", urlPasscode.trim().toUpperCase());
  }
  var initialPasscode = urlPasscode ? urlPasscode.trim().toUpperCase() : localStorage.getItem("w888_passcode") || null;
  window.__w888_online = true;
  var container = document.getElementById("root");
  var root = (0, import_client.createRoot)(container);
  root.render(/* @__PURE__ */ (0, import_jsx_runtime4.jsx)(WorldView, { folderPath: "", dc: window.dc, initialPasscode }));
})();
